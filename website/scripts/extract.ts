...
// Both version numbers, read from the files that declare them.
//
// The site quoted neither, so a reader had no way to tell which release the sample values
// on it came from — and the samples move with the *corpus*, not with the package, which is
// the number a person keeping generated fixtures actually needs. Printing them is the
// cheap half of documentation versioning and answers the question the expensive half
// exists for.
const declaredVersion = /static let version = "([^"]+)"/.exec(
  readFileSync(join(REPO, 'Sources', 'Decoy', 'Decoy.swift'), 'utf8'))?.[1] ?? 'unknown'
const corpusVersion = JSON.parse(
  readFileSync(join(REPO, 'Tools', 'adapters', 'corpus-version.json'), 'utf8')).version
...
Decoy \`${declaredVersion}\`, corpus \`${corpusVersion}\`. Every sample below was produced by
