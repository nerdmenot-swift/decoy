...
54 sources. 49 are fetched from a pinned URL and verified against an SRI
integrity hash, so a changed upstream fails the build rather than quietly altering
everyone's fixtures.

The remaining 5 have no file to hash. Three answer a query rather than
publishing a file — `wikidata`, `ssb-fornavn` and `surs-imena` — so the query is run
deliberately with `decoy-fetch` and its result committed, which anyone can re-run and
diff. Two are written here, and say so.
...
| `chinesename` | [chinesename (Chinese name statistics)](https://github.com/gumblex/chinesename) | `MIT` | 2026-08-17 |
...
| `naamapadam` | [Naamapadam (AI4Bharat) v1.0](https://huggingface.co/datasets/ai4bharat/naamapadam) | `CC0-1.0` | 2026-08-24 |
...
| `popular-names` | [Popular Names by Country](https://github.com/sigpwned/popular-names-by-country-dataset) | `CC0-1.0` | 2026-08-24 |
...
| `vietnamese-namedb` | [Vietnamese Name Database (Tu dien Ho Ten trong Viet Nam)](https://github.com/duyet/vietnamese-namedb) | `MIT` | 2026-08-17 |
