---
title: Sources
description: Every source the corpus is built from, with licence and retrieval date.
---

<!-- Generated by website/scripts/extract.ts. Do not edit by hand. -->

53 sources. 47 are fetched from a pinned URL and verified against an SRI
integrity hash, so a changed upstream fails the build rather than quietly altering
everyone's fixtures.

The remaining 6 have no file to hash. Three answer a query rather than
publishing a file — Wikidata over SPARQL, and the Norwegian and Slovenian statistics offices
over PxWeb — so the query is run deliberately with `decoy-fetch` and its result committed,
which anyone can re-run and diff. Two are written here, and say so.

| ID | Source | Licence | Retrieved |
|---|---|---|---|
| `airport-data` | [airport-data 1.0.1 (OpenFlights-derived)](https://github.com/jbrooksuk/JSON-Airports) | `Unlicense` | 2026-08-07 |
| `az-adlar` | [Azərbaycan adları (Azərbaycan Respublikasının Ədliyyə Nazirliyi)](https://opendata.az/dataset/azerbaycan-adlari) | `CC0-1.0` | 2026-08-10 |
| `cbs-shemot` | [השמות הנוכחיים של ילידי 1949-2024 (הלשכה המרכזית לסטטיסטיקה)](https://www.cbs.gov.il/he/mediarelease/Pages/2025/391.aspx) | `LicenseRef-DataGovIL-1.0` | 2026-08-10 |
| `chinesename` | [chinesename (Chinese name statistics)](https://github.com/gumblex/chinesename) | `MIT` | 2026-08-17 |
| `cities-json` | [cities.json 1.1.61 (GeoNames-derived)](https://github.com/lutangar/cities.json) | `CC-BY-4.0` | 2026-08-07 |
| `cldr-48` | [Unicode CLDR 48.2.0](https://github.com/unicode-org/cldr-json) | `Unicode-3.0` | 2026-08-06 |
| `common-knowledge` | [Widely-known facts, recorded without a single citable registry](https://github.com/nerdmenot-swift/decoy) | `public-facts` | 2026-08-12 |
| `decoy-authored` | [Decoy's own data](https://github.com/nerdmenot-swift/decoy) | `Apache-2.0` | 2026-08-09 |
| `dvv-etunimet` | [Etunimitilasto (Digi- ja väestötietovirasto), Finland](https://avoindata.suomi.fi/data/fi/dataset/none) | `CC-BY-4.0` | 2026-08-10 |
| `gender-by-name` | [Gender by Name (UCI Machine Learning Repository)](https://archive.ics.uci.edu/dataset/591/gender+by+name) | `CC-BY-4.0` | 2026-08-09 |
| `gleif-elf` | [ISO 20275 Entity Legal Forms code list (GLEIF), version 1.6](https://www.gleif.org/en/about-lei/code-lists/iso-20275-entity-legal-forms-code-list) | `public-facts` | 2026-08-10 |
| `iana-http-status` | [IANA HTTP Status Code Registry](https://www.iana.org/assignments/http-status-codes) | `public-facts` | 2026-08-09 |
| `iana-jose` | [IANA JSON Object Signing and Encryption (JOSE) Algorithms](https://www.iana.org/assignments/jose) | `public-facts` | 2026-08-09 |
| `iana-tld` | [IANA Root Zone Database](https://www.iana.org/domains/root/db) | `public-facts` | 2026-08-08 |
| `iana-tzdb` | [IANA Time Zone Database 2026b](https://www.iana.org/time-zones) | `public-domain` | 2026-08-06 |
| `ine-apellidos` | [Frecuencia de apellidos (Instituto Nacional de Estadística)](https://www.ine.es/dyngs/INEbase/es/operacion.htm?c=Estadistica_C&cid=1254736177009) | `CC-BY-4.0` | 2026-08-19 |
| `ine-nombres` | [Nombres por edad media (Instituto Nacional de Estadística), census 01/01/2025](https://www.ine.es/daco/daco42/nombyapel/nombyapel2.htm) | `CC-BY-4.0` | 2026-08-10 |
| `insee-prenoms` | [INSEE — Fichier des prénoms depuis 1900](https://www.insee.fr/fr/statistiques/7633685) | `LicenseRef-EtalabOpenLicence-2.0` | 2026-08-09 |
| `iso-4217-six` | [ISO 4217 currency list (SIX Group), published 2026-01-01](https://www.six-group.com/en/products-services/financial-information/data-standards.html) | `public-facts` | 2026-08-06 |
| `kosis-surnames` | [성씨별 인구 (Statistics Korea, Population and Housing Census)](https://kosis.kr) | `LicenseRef-KOGL-Type-1` | 2026-08-19 |
| `ldnoobw` | [List of Dirty, Naughty, Obscene and Otherwise Bad Words](https://github.com/LDNOOBW/List-of-Dirty-Naughty-Obscene-and-Otherwise-Bad-Words) | `CC-BY-4.0` | 2026-08-09 |
| `libaddressinput` | [Google libaddressinput (countryinfo.txt)](https://github.com/google/libaddressinput) | `Apache-2.0` | 2026-08-09 |
| `libphonenumber` | [Google libphonenumber (PhoneNumberMetadata.xml)](https://github.com/google/libphonenumber) | `Apache-2.0` | 2026-08-09 |
| `lilak` | [Lilak Persian spell-checking dictionary 3.3](https://github.com/b00f/lilak) | `Apache-2.0` | 2026-08-07 |
| `linguist` | [linguist-languages 9.4.0](https://github.com/github-linguist/linguist) | `MIT` | 2026-08-07 |
| `mime-db` | [mime-db 1.54.0](https://github.com/jshttp/mime-db) | `MIT` | 2026-08-06 |
| `moi-surnames` | [全國姓名統計 — 姓氏排名 (內政部, Ministry of the Interior, Taiwan)](https://data.gov.tw/dataset/126774) | `LicenseRef-OGDL-Taiwan-1.0` | 2026-08-10 |
| `omw-cmn` | [Chinese Open Wordnet (OMW 2.0)](https://omwn.org/) | `LicenseRef-ChineseOpenWordNet` | 2026-08-07 |
| `omw-da` | [DanNet (OMW 2.0)](https://omwn.org/) | `LicenseRef-DanNet-1.0` | 2026-08-07 |
| `omw-el` | [Greek Wordnet (OMW 2.0)](https://omwn.org/) | `Apache-2.0` | 2026-08-07 |
| `omw-en` | [Princeton WordNet (via OMW) (OMW 2.0)](https://omwn.org/) | `WordNet-3.0` | 2026-08-07 |
| `omw-es` | [Multilingual Central Repository (Spanish) (OMW 2.0)](https://omwn.org/) | `WordNet-3.0 AND CC-BY-3.0` | 2026-08-07 |
| `omw-fi` | [FinnWordNet (OMW 2.0)](https://omwn.org/) | `WordNet-3.0 AND CC-BY-3.0` | 2026-08-07 |
| `omw-he` | [Hebrew Wordnet (OMW 2.0)](https://omwn.org/) | `LicenseRef-HebrewWordNet` | 2026-08-07 |
| `omw-hr` | [Croatian Wordnet (OMW 2.0)](https://omwn.org/) | `CC-BY-3.0` | 2026-08-07 |
| `omw-id` | [Wordnet Bahasa (Indonesian) (OMW 2.0)](https://omwn.org/) | `MIT` | 2026-08-07 |
| `omw-it` | [MultiWordNet (Italian) (OMW 2.0)](https://omwn.org/) | `CC-BY-3.0` | 2026-08-07 |
| `omw-ja` | [Japanese Wordnet (OMW 2.0)](https://omwn.org/) | `LicenseRef-JapaneseWordNet` | 2026-08-07 |
| `omw-nb` | [Norwegian Wordnet (Bokmal) (OMW 2.0)](https://omwn.org/) | `LicenseRef-NWN` | 2026-08-07 |
| `omw-pl` | [plWordNet (OMW 2.0)](https://omwn.org/) | `WordNet-3.0` | 2026-08-07 |
| `omw-sv` | [WordNet-SALDO (OMW 2.0)](https://omwn.org/) | `CC-BY` | 2026-08-07 |
| `onet` | [O*NET 30.3 Database (US Department of Labor)](https://www.onetcenter.org/database.html) | `CC-BY-4.0` | 2026-08-09 |
| `ons-baby-names` | [Baby names in England and Wales 1996 to 2025 (Office for National Statistics)](https://www.ons.gov.uk/peoplepopulationandcommunity/birthsdeathsandmarriages/livebirths/datasets/babynamesinenglandandwalesfrom1996) | `LicenseRef-OGL-3.0` | 2026-08-10 |
| `pesel-imiona` | [Lista imion występujących w rejestrze PESEL (Ministerstwo Cyfryzacji)](https://dane.gov.pl/pl/dataset/1667) | `CC0-1.0` | 2026-08-10 |
| `pubchem` | [PubChem periodic table (NCBI/NIH)](https://pubchem.ncbi.nlm.nih.gov/periodic-table/) | `public-domain` | 2026-08-07 |
| `scb-namn` | [Namn med minst två bärare (Statistiska centralbyrån), 31 december 2022](https://www.scb.se/hitta-statistik/statistik-efter-amne/befolkning-och-levnadsforhallanden/ovrigt/namnstatistik/) | `CC-BY-4.0` | 2026-08-10 |
| `ssb-fornavn` | [Navn, etter fornavn (Statistisk sentralbyrå), table 10501](https://www.ssb.no/statbank/table/10501) | `CC-BY-4.0` | 2026-08-10 |
| `surs-imena` | [Prva imena (Statistični urad Republike Slovenije), tables 05X1005S and 05X1010S](https://pxweb.stat.si/SiStat/en/Podrocja/Index/100/prebivalstvo) | `CC-BY-4.0` | 2026-08-10 |
| `unicode-emoji` | [Unicode Emoji 16.0 (emoji-test.txt)](https://www.unicode.org/Public/emoji/16.0/) | `Unicode-3.0` | 2026-08-08 |
| `us-census-surnames` | [US Census 2010 surname file](https://www.census.gov/topics/population/genealogy/data/2010_surnames.html) | `public-domain` | 2026-08-07 |
| `vietnamese-namedb` | [Vietnamese Name Database (Tu dien Ho Ten trong Viet Nam)](https://github.com/duyet/vietnamese-namedb) | `MIT` | 2026-08-17 |
| `whitakers-words` | [Whitaker's Words (DICTLINE.GEN)](https://github.com/mk270/whitakers-words) | `LicenseRef-WhitakersWords` | 2026-08-09 |
| `wikidata` | [Wikidata (given names, surnames and colour names by language)](https://query.wikidata.org/) | `CC0-1.0` | 2026-08-09 |

`public-facts` and `public-domain` are not SPDX identifiers. The first records that the
extracted content is facts nobody authored — that Antigua and Barbuda has the ISO code
`AG` — and so has no author to license it. The second is a positive statement by the
publisher.
