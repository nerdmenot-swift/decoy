...
**Built so far** — thirty-six adapters, fifty-three sources, and no faker-js:
...
| Provenance | A source/license table, referenced by ID | **In use** — 53 sources, attributed by nearest claimed ancestor, and the origin of `NOTICE` |
...
2. ~~Core generators against the faker-derived corpus~~ — **done** (315 methods across 28
   namespaces; 297 across 27 without Foundation, which gates the `date` namespace)
...
8. ~~Coverage gates reach threshold → drop the faker-js adapter and its descriptor~~ —
   ...
   **Tracked rather than hoped for.** `decoy-validate` prints what faker-js still supplies
   ...
   | `internet.http_status_code.*` | faker, 63 codes | IANA's registry, 62 current ones with the reserved ranges excluded |
   | `internet.jwt_algorithm` | faker, 13 | IANA JOSE, 38 — `alg` usage only, since `enc` names a content encryption method a JWT header never carries |
   | `date.{month,weekday}.*_context` | faker, 4-6 locales | CLDR stand-alone forms in every locale that has dates |
