...
Then it builds `decoy-inspect`, `decoy-validate` and `decoy-compile-corpus` for macOS
(universal), Linux (static, runs with no Swift installed) and Windows, publishes
`SHA256SUMS`, and attaches signed build provenance:

```
gh attestation verify decoy-tools-1.1.0-macos-universal.tar.gz --repo nerdmenot-swift/decoy
```

Unlike a checksum file, that cannot be regenerated by whoever replaced the download.
...
- **Signed build provenance** (`actions/attest-build-provenance`) — now standard for npm,
  PyPI and Homebrew, still rare for Swift.
