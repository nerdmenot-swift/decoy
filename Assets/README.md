...
Everything here is generated by `build.mjs` from computed geometry. **Do not edit the
SVGs or PNGs by hand**; change a ratio in the script and re-run it:

```
node Assets/build.mjs
```

Node built-ins only — no npm, no package manifest, no `node_modules`, the same rule the
adapter pipeline follows. That is why the PNG encoder and the polygon rasteriser are in
the script rather than being `sharp`: the point of a toolchain that takes no
dependencies is not to take one for a favicon.
